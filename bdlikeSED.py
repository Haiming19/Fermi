...
class bdlikeInput:
	...
	def __init__(self,LIKE,ft1,cc,ft2,SrcName,model='',nbins=5,phCorr=1.0):
...
print 'This is bdlikeSED version 12.'
...
def maxEnergy(likeIn,file):
	...
	psf={} #make psf instances
	psf[0]=irfs[0].psf()
	psf[1]=irfs[1].psf()
	ra=likeIn.bdAn.model.srcs[likeIn.source].src.getSrcFuncs()['Position'].getParamValue('RA')
	dec=likeIn.bdAn.model.srcs[likeIn.source].src.getSrcFuncs()['Position'].getParamValue('DEC')
	EMax=[]
	Sep=angsep(ra,dec,RA,DEC)
	fangInt=psf[0].angularIntegral
	bangInt=psf[1].angularIntegral
	for s,c,r,d,e,t,p in zip(Sep,CNVTYPE,RA,DEC,ENERGY,THETA,PHI):
		if(c==0):
...
def angsep(ra1,dec1,ra2,dec2):
	...
	cdiff=num.cos(diff)
	SEP=num.arccos([cdec1*cd2*cdf+sdec1*sd2 for cd2,cdf,sd2 in zip(cdec2,cdiff,sdec2)]) #returns values between 0 and pi radians
...
def runLike(likeIn,ecent,ftol,tslim,ttype,opt,rescaleAll,lastbinUL,wx):
	...
	for i in range(0,likeIn.nbins): #first, set up for a likelihood run
		...
		if stype=='PowerLaw':
			...
			if fail==0:
				for src in band_like.sourceNames():
					if src!=Src:
						par=band_like.normPar(src)
						if band_like[src].type=='PointSource' and par.isFree() and band_like.Ts(src)<=0:
							band_like.deleteSource(src)
							print "   -Removing %s from the model" %src
			Upper=getParamIndx(band_like,Src,'UpperLimit')
			Lower=getParamIndx(band_like,Src,'LowerLimit')
...
def plotSED(sed,plot):
	...
	GeVBands=[float(x/1000.) for x in minEs] #put bands for data points in units of GeV
	...
	if len(sed.data[3])==0:#no index values filled for PowerLaw models
		GeVfluxPts=[float(x*1000.) for x in fluxPts if x!=0]
		GeVfluxErrs=[float(x*1000.) for x,X in zip(fluxErrs,fluxPts) if X!=0]
	else:#for PowerLaw2 models, use index information
		newfluxPts=[]
		newfluxErrs=[]
	...
	cntLegend.AddEntry(cnt,"4FGL J0521.2+1637",'p')
	cntLegend.AddEntry(modcnt,"Best-fit Model",'l')
	...
	flxLegend.AddEntry(flx,"4FGL J0521.2+1637",'p')
	flxLegend.AddEntry(modflx,"Best-fit Model",'l')
...
def writeSpecFits(sed,modelEbins,modelEcent,minEs,maxEs,GeVfluxPts,GeVfluxErrs,ecent,esqf,esqferr):
	...
	hdulist.writeto('%s_%ibins_bdlikeSEDout.fits' %(sed.likeIn.source.replace(' ','_'),sed.likeIn.NBins),output_verify='ignore',clobber='yes') #write the file
	return
...
def residualsPlot(sed,plot):
	...
	modelErrs=[]
	#get the model
	mysrc=pyLike.PointSource_cast(sed.likeIn.bdAn[sed.likeIn.source].src)
	...
	resids=[]
	residErrs=[]
	if len(sed.data[3])==0:#if PowerLaw model used, no index info filled
	...
	else:#PowerLaw2 model uses the index info
		newfluxPts=[]
		newfluxErrs=[]
		for m,M,f,e,g,c in zip(Emins,Emaxs,fluxPts,fluxErrs,sed.data[3],ecent):
			newfluxPts+=[f*(-g+1)*(c*1000.)**(-g)/(M**(-g+1)-m**(-g+1))] #works out to be prefactor assuming Scale parameter (E0) of c (center energy of bin)
			newfluxErrs+=[e*(-g+1)*(c*1000.)**(-g)/(M**(-g+1)-m**(-g+1))]#similar for the errors
		#for x,y,z,e in zip(Emins,Emaxs,fluxPts,fluxErrs):
			#newfluxPts+=[z/(y-x)]
			#newfluxErrs+=[e/(y-x)]
...
def fullLike(likeIn,CoVar,FTol,minE,maxE,wx=False):
	...
	#do the full fit
	try:
		bdAn.fit(tol=FTol,covar=CoVar,verbosity=0)
	except:
		try:
			print 'Trying lower tolerance of %s for full fit.' %FTol/10.
			bdAn.fit(tol=FTol/10.,covar=CoVar,verbosity=0)
		except:
			try:
	...
	if(likeIn.bdAn.covariance is None):
		...
		for src in bdAn.sourceNames():
			parNames=pyLike.StringVector()
			bdAn[src].src.spectrum().getFreeParamNames(parNames)
			for par in parNames:
				par_index_map['::'.join((src,par))]=indx
				indx +=1
		srcPars=pyLike.StringVector()
		bdAn[likeIn.source].src.spectrum().getFreeParamNames(srcPars)
		...
		for x in centEs:
			...
			val=num.sqrt(num.dot(partials,num.dot(cov,partials))) #these should come out same as the model so convert to ph/cm^2/s/GeV as well
			bt+=[float(1000.*val)]
	myfile=open('bdlikeSED_%s_fullFitout.txt'%likeIn.source.replace(' ','_'),'w')
	print 'Full energy range model for %s:' %likeIn.source
	myfile.write('Full energy range model for %s:\n' %likeIn.source)
	print bdAn[likeIn.source]
	myfile.write('%s\n'%bdAn[likeIn.source])
	if bdAn.covariance is None:
		myfile.write('(Covariance Matrix not calculated)\n')
		print 'Flux %.1f-%.1f GeV %.1e cm^-2 s^-1' %(likeIn.obsEBounds[0]/1000.,likeIn.obsEBounds[1]/1000.,bdAn.flux(likeIn.source,emin=likeIn.obsEBounds[0],emax=likeIn.obsEBounds[1]))
		myfile.write('Flux %.1f-%.1f GeV %.1e cm^-2 s^-1\n' %(likeIn.obsEBounds[0]/1000.,likeIn.obsEBounds[1]/1000.,bdAn.flux(likeIn.source,emin=likeIn.obsEBounds[0],emax=likeIn.obsEBounds[1])))
	else:
		print 'Flux %.1f-%.1f GeV %.1e +/- %.1e cm^-2 s^-1' %(likeIn.obsEBounds[0]/1000.,likeIn.obsEBounds[1]/1000.,bdAn.flux(likeIn.source,emin=likeIn.obsEBounds[0],emax=likeIn.obsEBounds[1]),bdAn.fluxError(likeIn.source,emin=likeIn.obsEBounds[0],emax=likeIn.obsEBounds[1]))
		myfile.write('Flux %.1f-%.1f GeV %.1e +/- %.1e cm^-2 s^-1\n' %(likeIn.obsEBounds[0]/1000.,likeIn.obsEBounds[1]/1000.,bdAn.flux(likeIn.source,emin=likeIn.obsEBounds[0],emax=likeIn.obsEBounds[1]),bdAn.fluxError(likeIn.source,emin=likeIn.obsEBounds[0],emax=likeIn.obsEBounds[1])))
	print 'Test Statistic',bdAn.Ts(likeIn.source)
	myfile.write('Test Statistic %.2f'%bdAn.Ts(likeIn.source))
	myfile.close()
	if wx:
		bdAn.writeXml('%s_fullErange_fitmodel.xml'%likeIn.source.replace(' ','_'))
	return bt,spec
